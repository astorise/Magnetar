...
use crate::tensor::{
    HostTensor, ReferenceCpuError, ReferenceCpuErrorCode, TensorLifecycleState, TensorReadiness,
    TensorResource,
};
use std::{
...
///
/// `head_count = 1` reproduces this function's pre-`make-first-native-cuda-
/// hot-path-device-resident` single-block behavior exactly (`head_width`
/// becomes `cols`) -- mirrors `providers/cpu::rope`'s identical fix; this is
/// magnetar-runtime's own in-crate copy (`reference_cpu.rs`'s module doc:
/// "in-crate test double, not the sole implementation"), independently
/// maintained.
///
...
/// projection bias: `[seq, dim] + [dim] -> [seq, dim]`; `providers/cpu`'s
/// externalized copy of this function carries the identical extension).
/// Purely additive over the prior same-shape-only behavior.
...
impl ReferenceCpuExecutor {
    ...
    /// contract (`generalize-first-native-provider-dispatch`'s "Provider
    /// TensorValue Mutation Error Channel"), not because this Provider can
    /// fail here.
