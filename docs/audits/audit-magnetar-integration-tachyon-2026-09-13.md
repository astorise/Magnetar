...
6. **`add-second-component-fixture-for-registry-multiplicity-proof`** : un second Component WASM réel (`synthetic-minimal.component.wasm`), structurellement distinct (graphe `embedding -> rmsnorm -> matmul`, aucune couche décodeur), construit avec la même chaîne d'outils réelle que le Component Qwen de production (`cargo build --target wasm32-unknown-unknown` puis `wasm-tools component new`), enregistré aux côtés du Component Qwen réel dans le même test consolidé -- preuve que le registre générique sert bien deux Components distincts simultanément (graphes différents, aucune perturbation mutuelle), et non un simple alias du singleton Qwen historique -- clôt MAG-07.

**MAG-05** : structurellement déjà satisfait côté Magnetar -- chaque commit poussé sur `main` (dont celui de ce chantier) déclenche la Quality CI complète (`cargo check`/`clippy -D warnings`/`cargo test`/`cargo deny`/MSRV/wasm32 Component runtime/provider conformance/OpenSpec/integration tests), vérifiée verte avant et après chaque chantier de cette liste. Ce qui reste hors de portée de ce dépôt : que Tachyon consomme effectivement un commit `main` post-réconciliation (MAG-04) plutôt qu'une lignée divergente -- une action côté Tachyon, pas une correction supplémentaire côté Magnetar.

**Scope de l'audit Tachyon (MAG-01 à MAG-07) intégralement traité côté Magnetar.**

7. **`add-inference-components-integration-test-and-ci-coverage`** : clôt la dernière note laissée ouverte ci-dessus. Un bundle Hugging Face réel et intégralement spécifié (chaque tenseur attendu par le graphe du Component Qwen réel, aux formes exactes pré-transposition) est construit dans un répertoire temporaire et chargé via `LoadedInferenceComponent::load`, puis une génération réelle est exécutée via `invoke_payload` -- l'orchestration propre à cette crate (sélection du format, construction du tokenizer, enregistrement du Component, évaluation du trust, `load_with_component`) est désormais prouvée de bout en bout, pas seulement le chemin `magnetar-runtime` sous-jacent. En construisant ce test est apparue une découverte distincte et plus sérieuse : `inference-components` a son propre `[workspace]` (comme `magnetar-cli`, sorti du workspace racine pour la même raison de dépendance aux submodules) mais, contrairement à `magnetar-cli`, n'était câblé dans aucun job CI -- la crate la plus directement revue par cet audit n'était construite ni testée nulle part en CI. Corrigé en l'ajoutant au job `submodule-integration`, avec les mêmes étapes format/clippy que `magnetar-cli`, vérifié vert sur le vrai runner Linux.

Reste, hors du périmètre strict de cet audit mais noté pour mémoire : l'équivalent GGUF de ce test de bout en bout (la branche `is_gguf` elle-même reste couverte séparément par les tests de `loaders/gguf` et les tests `magnetar-runtime` singleton/Component-nommé).
