...
## Purpose
Defines the Capability a Model Component uses to export prefill/decode graph semantics to the Runtime instead of the Runtime synthesizing model-family-specific graphs internally: portable node/Operator identity, Runtime-owned Tensor descriptors and weight references, and KV logical resources the Runtime issues. `add-real-multi-device-model-instance-placement` extended it (`model-component-graph.wit` `1.2.0` -> `1.3.0`, purely additive) with `build-prefill-graph-segment`/`build-decode-graph-segment`: building a graph for one real decoder-layer range instead of always the whole stack, the capability real per-Device pipeline placement for a production `ModelInstance` is built on.
## Requirements
