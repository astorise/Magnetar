# cli-boundary Specification

## Purpose
Define the authority boundary between `magnetar-cli`, the first-party
client/workspace/tool/agent runtime, and `magnetar-runtime`, the
inference-only runtime. This capability specifies which responsibilities —
workspace and file access, Git, network, secrets, tool execution, shell and
process execution, agent orchestration, prompt/context assembly,
configuration, model alias resolution, sessions, streaming, cancellation,
diagnostics, and observability — are owned by the CLI rather than the Runtime,
so that CLI authority is never implicitly delegated to Runtime and Runtime
remains a clean, inference-only substrate called only through the Runtime
Inference API.
## Requirements
