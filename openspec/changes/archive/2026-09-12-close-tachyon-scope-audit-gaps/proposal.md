...
- Production generation (`run_production_qwen_generation`/`_for_provider`) gains a `PromptInput`/`ChatTemplateFormatter`-aware variant so a caller can render `PromptInput::ChatMessages` through the artifact's own real chat template, instead of every production generation call being hardcoded to a bare `PromptInput::PlainText` prompt string regardless of what the ingested manifest declares. (Investigation found `magnetar-cli`'s existing chat path is bound only to the separate `qwen-test` Component fixture, with no existing connection to a production-loaded instance at all -- there is no existing CLI chat-formatter selection to extend without also building a new "run generation against a locally loaded production model" CLI feature, which is a materially larger, separate scope than closing this gap; see Non-Goals.)
...
- `magnetar-cli`: unchanged. `pipeline.rs`'s `CliChatTemplateFormatter` and `ChatSession` remain bound only to the `qwen-test` Component fixture; there is no existing CLI path that runs generation against a production-loaded instance for this change to extend (see Non-Goals).
...
- Also out of scope: wiring `magnetar-cli` to run generation against a production-loaded instance at all (today `magnetar model load --file` only proves loading; no CLI command then generates from that instance). This is a real, separate gap this session's investigation found, but building it is a new CLI feature, not a fix to the two audit-identified gaps this change closes.
