...
- [x] 3.1 `register_inference_component_artifact_enforces_trust_is_idempotent_and_matches_the_singleton_path`: a single sequential test covering trust rejection (empty store), trust acceptance (real digest matches an independently computed `ComponentDigest::sha256`), idempotent re-registration, and the load-bearing correctness proof (graphs built via the new generic path have identical operator-sequence hashes -- not just node counts -- to graphs built via the pre-existing hardcoded singleton path, for the identical real `qwen-real.component.wasm` fixture bytes).
- [x] 3.2 **Real CI failure caught and fixed**: this was originally three/four separate `#[test]` functions. A real CI run (not local) failed `register_inference_component_artifact_computes_real_digest_and_enforces_caller_trust`'s untrusted-rejection assertion -- Rust's default parallel test execution let a *different* test's trusted registration for the identical fixture bytes (same digest, since content-derived) populate the shared process-wide registry first, so the deliberately-untrusted attempt hit the idempotent-return fast path before trust was ever re-evaluated. Fixed by consolidating every assertion touching that one shared digest into one sequential test, eliminating the race by construction rather than adding test-only synchronization. Documented as a real, instructive incident, not glossed over.
- [x] 3.3 `build_first_native_graphs_from_named_component_fails_closed_for_an_unregistered_digest`: immune to the same race by construction (uses a distinct, never-registered all-zero digest).
- [x] 3.4 Full regression: `cargo test -p magnetar-runtime --lib` (1261 passed after consolidation, down from the initially-reported 1262 since two tests merged into one; zero regressions), `cargo clippy -p magnetar-runtime --all-targets -- -D warnings` clean, `cargo fmt --check` clean.
- [x] 3.5 Fixed an unrelated compile break surfaced by reconciling `codex/tachyon-component-boundary` with `main`: `integration-tests/production-loading`'s 14 `ProductionGenerationRequest` struct literals were missing the `max_generation_millis` field that branch added; added `max_generation_millis: None` to each. Verified the crate builds clean.
