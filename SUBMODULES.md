...
| This branch | `providers/cuda` | `557ceaa` or later | Requires `magnetar-runtime`'s Provider/Device/Kernel/Tensor/Memory contracts and the same `write_tensor_value`/`write_tensor_value_admitted` error-channel shape as `providers/cpu` above; never referenced by `magnetar-runtime`. Its own kernel/Memory Manager/conformance behavior was verified against real GPU hardware (RTX 3070 Ti Laptop GPU), not just compiled -- see `implement-cuda-provider-baseline` and `enable-device-resident-kernel-chaining`. `557ceaa` specifically: replaces host-resident storage with a real, persistent device allocation table and explicit-only host/device data movement (`enable-device-resident-kernel-chaining`) -- `7ac13b1` and earlier commits are correct but do not satisfy the CUDA Provider baseline's own "Explicit Data Movement"/"Memory Manager Integration" spec requirements literally (device residency was claimed to the Memory Manager but not physically real) |
