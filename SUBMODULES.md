...
| --- | --- | --- | --- |
| This branch (`make-first-native-datapath-authoritative`) | `components/qwen` | `f91290c` or later | Requires `magnetar:model-component-graph@1.0.0`; is the exclusive production graph source under the strict path (`reach-architecture-freeze-1` task 11.5). `f91290c` gives `rope_q`/`rope_k` a native `head_count` attribute (Q = `ATTENTION_HEAD_COUNT`, K = `KV_HEAD_COUNT`), matching `providers/cpu`/`providers/cuda`'s own `76f082b`/`f8eb5eb` -- required for `make-first-native-cuda-hot-path-device-resident`'s Runtime-side `head_count` fallback removal; `magnetar-runtime/fixtures/components/qwen-real.component.wasm` (the checked-in, compiled artifact this crate's own `#[cfg(test)]` builds and the `integration-tests/cuda-first-native` crate both use) was regenerated from this exact commit, digest updated accordingly |
